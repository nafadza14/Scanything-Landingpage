    ...
    <section id="trackers" className="py-32 bg-[#FDFDFD] overflow-hidden relative">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="grid lg:grid-cols-2 gap-10 max-w-7xl mx-auto">
          {/* Food Tracker Card - Removed Border, Increased Curves */}
          <div className="group bg-white rounded-[6rem] p-16 shadow-premium transition-all duration-700 hover:-translate-y-3 hover:shadow-floating overflow-hidden relative border-none">
            ...
            <div className="bg-[#F9FAFB] rounded-[4rem] p-12 shadow-inner relative overflow-hidden border-none">
          ...
          {/* Skin Tracker Card - Removed Border, Increased Curves */}
          <div className="group bg-white rounded-[6rem] p-16 shadow-premium transition-all duration-700 hover:-translate-y-3 hover:shadow-floating overflow-hidden relative border-none">
            <div className="absolute top-0 right-0 w-64 h-64 bg-primary/5 rounded-full -mr-20 -mt-20 blur-3xl group-hover:bg-primary/10 transition-colors"></div>
            ...
            <div className="bg-[#F9FAFB] rounded-[4rem] p-12 shadow-inner relative border-none">
                <div className="flex justify-between items-center mb-10">
                  <div className="flex -space-x-4">
                    {[1,2,3].map(i => (
                      <div key={i} className="w-14 h-14 rounded-full border-[5px] border-white bg-gray-100 overflow-hidden shadow-sm transition-transform hover:scale-110">
                        <img src={`https://i.pravatar.cc/100?img=${i+25}`} alt="Profile" className="w-full h-full object-cover" />
                      </div>
                    ))}
                    <div className="w-14 h-14 rounded-full border-[5px] border-white bg-primary flex items-center justify-center text-[10px] text-white font-black shadow-lg">AI</div>
                  </div>
                ...
                <div className="grid grid-cols-7 gap-4">
                  {[1,1,1,1,1,1,0].map((active, i) => (
                    <div key={i} className={`h-12 rounded-3xl flex items-center justify-center transition-all duration-300 ${active ? 'bg-primary text-white shadow-pink-glow scale-105' : 'bg-white text-gray-200 border-none'}`}>
                      {active ? (
                ...
                <div className="mt-8 flex justify-between text-[10px] font-black text-gray-400 uppercase tracking-widest px-2">
